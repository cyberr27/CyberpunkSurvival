...
const TRADE_COOLDOWN_AFTER_CANCEL_MS = 30_000;
    ...
    ws.on("message", async (message) => {
            ...
            if (now < cooldownEnd) {
              ...
              if (ws.readyState === WebSocket.OPEN) {
                ws.send(
                  JSON.stringify({
                    type: "tradeError",
                    message: `Нельзя отправить запрос этому игроку ещё ${secondsLeft} сек`,
      ...
      async function processTradeCancelledQueue(ws) {
        ...
        while (ws.tradeCancelledQueue.length > 0) {
              ...
              const restoreOfferToInventory = (player, offer) => {
                ...
                offer.forEach((item) => {
                  ...
                  if (freeSlot !== -1) {
                    ...
                    console.warn(
                      `[Trade Cancel] Нет места для возврата ${type} игроку ${player.id || "unknown"}`,
                    );
                  }
                });
              };
          ...
          // Удаляем запрос, если он был
          tradeRequests.delete(tradeKey);

          // Рассылаем отмену обоим
          wss.clients.forEach((client) => {
            if (client.readyState === WebSocket.OPEN) {
              const clientId = clients.get(client);
      ...
      async function processTradeChatQueue(ws) {
        ...
        // Инициализируем per-websocket счётчики, если их ещё нет
        if (!ws.tradeChatTimestamps) {
          ws.tradeChatTimestamps = [];
        }
        if (!ws.tradeChatLastMessageTime) {
          ws.tradeChatLastMessageTime = 0;
        }
        ...
        while (ws.tradeChatQueue.length > 0) {
          ...
          if (data.fromId !== senderId) {
            console.warn(
              `[AntiCheat] Подмена fromId в tradeChatMessage: ${data.fromId} ≠ ${senderId}`,
            );
            continue;
          }
          ...
          if (ws.tradeChatTimestamps.length >= MAX_MESSAGES_PER_5SEC) {
            if (ws.readyState === WebSocket.OPEN) {
              ws.send(
                JSON.stringify({
                  type: "tradeChatError",
                  message: "Слишком быстро пишешь. Подожди пару секунд.",
                }),
              );
            }
            continue;
