    ...
    ws.on("message", async (message) => {
      ...
      if (data.type === "register") {
        ...
        if (
          ...
          (player.neonQuest.progress?.killMutants || 0) < 3
        ) {
          ws.send(
            JSON.stringify({
              type: "notification",
        ...
        // === ВЫДАЁМ НАГРАДУ ТУТ, ТОЛЬКО ОДИН РАЗ ===
        ...
        const killerId = data.killerId; // ← ЭТО КЛЮЧЕВОЕ: кто именно убил!
        const enemyType = data.enemyType;

        if (!killerId || !players.has(killerId)) return;

        const killer = players.get(killerId);

        // Только если у игрока активен квест Neon Alex — считаем мутантов
        if (
          killer.worldId === data.worldId &&
          killer.neonQuest?.currentQuestId === "neon_quest_1" &&
          enemyType === "mutant"
        ) {
          // Инициализируем прогресс, если его нет
          if (!killer.neonQuest.progress) {
            killer.neonQuest.progress = { killMutants: 0 };
          }

          killer.neonQuest.progress.killMutants =
            (killer.neonQuest.progress.killMutants || 0) + 1;

          // Сохраняем на сервере
          players.set(killerId, killer);
          userDatabase.set(killerId, killer);
          await saveUserDatabase(dbCollection, killerId, killer);

          // Отправляем ТОЛЬКО убийце обновлённый прогресс
          wss.clients.forEach((client) => {
            if (
              clients.get(client) === killerId &&
              client.readyState === WebSocket.OPEN
            ) {
              client.send(
                JSON.stringify({
                  type: "neonQuestProgress",
                  progress: killer.neonQuest.progress,
                })
              );
            }
          });
