    ...
    ws.on("message", async (message) => {
      ...
      if (data.type === "register") {
        ...
        if (item.type === "atom") {
          // Удаляем старый атом
          items.delete(data.itemId);
          atomItems.delete(player.worldId); // Если per-world

          // Спавним новый сразу (вызываем логику спавна; для простоты дублируем, или сделай функцию)
          const world = worlds.find((w) => w.id === player.worldId);
          if (world) {
            const type = "atom";
            ...
            if (attempts < maxAttempts) {
              ...
              atomItems.set(world.id, { itemId, lastMoveTime: currentTime });

              // Лог создания нового после поднятия
              console.log(
                `Новый Атом создан после поднятия в мире ${
                  world.id
                } на координатах x:${x}, y:${y} в ${new Date(
                  currentTime
                ).toISOString()}`
              );
