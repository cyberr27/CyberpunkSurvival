...
function runGameLoop(
  ...
  setInterval(() => {
    ...
    worlds.forEach((world) => {
      ...
      // Новая логика для Атомов (во всех мирах)
      const atomItems = Array.from(items.values()).filter(
        (item) => item.type === "atom" && item.worldId === world.id
      );

      if (atomItems.length === 0) {
        // Нет Атома — спавним новый
        ...
        if (attempts < maxAttempts) {
          ...
          console.log(
            `Создан новый Атом (${atomId}) в мире ${world.id} на x:${
              newAtom.x
            }, y:${newAtom.y} (время: ${new Date(currentTime).toISOString()})`
          );
          ...
          console.log(
            `Не удалось найти место для спавна Атома в мире ${world.id}`
          );
        }
      } else {
        // Есть Атом — проверяем время для перемещения
        atomItems.forEach((atom) => {
          if (currentTime - atom.spawnTime > 10 * 60 * 1000) {
            // 10 минут
            // Удаляем старый
            const atomId = Array.from(items.entries()).find(
              ([id, item]) => item === atom
            )[0];
            items.delete(atomId);
            console.log(
              `Атом (${atomId}) удалён из мира ${
                world.id
              } из-за таймаута (время: ${new Date(currentTime).toISOString()})`
            );
            wss.clients.forEach((client) => {
              if (
                client.readyState === WebSocket.OPEN &&
                ...
                client.send(
                  JSON.stringify({ type: "itemPicked", itemId: atomId })
                );
              }
            });

            // Спавним новый (рекурсивно вызовем логику, но лучше напрямую)
            const newAtomId = `atom_${Date.now()}`;
            let newX,
              newY,
              attempts = 0;
            do {
              newX = Math.random() * world.width;
              newY = Math.random() * world.height;
              attempts++;
            } while (
              checkCollisionServer(newX, newY) &&
            ...
            if (attempts < maxAttempts) {
              ...
              items.set(newAtomId, newAtom);
              console.log(
                `Перемещён Атом (${newAtomId}) в мире ${world.id} на x:${
                  newAtom.x
                }, y:${newAtom.y} (время: ${new Date(
                  currentTime
                ).toISOString()})`
              );
              ...
              console.log(
                `Не удалось найти новое место для Атома в мире ${world.id}`
              );
            }
          }
        });
