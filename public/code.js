...
const frameDuration = 800; // Длительность одного кадра в миллисекундах (настраиваемая скорость анимации)
...
function updateResources() {
  const me = players.get(myId);
  if (!me) return;
  if (me.steps % 300 === 0) me.energy = Math.max(0, me.energy - 1);
  if (me.steps % 150 === 0) me.food = Math.max(0, me.food - 1);
  if (me.steps % 75 === 0) me.water = Math.max(0, me.water - 1);
  if (
