...
function createNPC(id, x, y, name) {
    npcs.set(id, {
        ...
        y,
        name,
        hasInteracted: false, // Флаг первого взаимодействия
        currentQuestId: null, // ID текущего квеста
        collectedItems: {}, // Отслеживание собранных предметов
    });
}
...
function checkNPCProximity() {
    ...
    if (closestNPC && !isQuestDialogOpen) {
        openQuestDialog(closestNPC);
    } else if (!closestNPC && isQuestDialogOpen) {
        closeQuestDialog();
    }
}
...
function openQuestDialog(npc) {
    ...
    if (!npc.hasInteracted) {
        ...
        // Повторное взаимодействие — показываем список квестов
        showQuestList(npc);
    }
}
    ...
    dialogContainer.innerHTML = `
        ...
        <ul id="questList" class="quest-list">
            ${questList
                .map(
                    (quest) => `
                <li class="${quest.id === currentQuest.id ? 'active' : ''}">
...
function checkQuestProgress(item) {
    ...
    if (item.type === targetType) {
        activeNPC.collectedItems[targetType] = (activeNPC.collectedItems[targetType] || 0) + 1;
        console.log(`Собран предмет ${targetType}, прогресс: ${activeNPC.collectedItems[targetType]}/${currentQuest.target.quantity}`);
...
function completeQuest() {
    ...
    if (balyarySlot !== -1) {
        inventory[balyarySlot].quantity = (inventory[balyarySlot].quantity || 1) + reward.quantity;
    } else {
    ...
    // Обновляем квест
    const availableQuests = QUESTS.filter((quest) => quest.id !== currentQuest.id);
    const newQuest = availableQuests[Math.floor(Math.random() * availableQuests.length)];
