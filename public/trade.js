  ...
  switch (data.type) {
    ...
    case "tradeItemPlaced":
      if (
        ...
        // Обновляем инвентарь, если предмет был помещён другим игроком
        if (data.playerId !== players.get(myId).id) {
...
function resetInventoryButtons() {
  ...
  // Устанавливаем обработчики в зависимости от состояния
  if (isTradeWindowOpen) {
    ...
    dropBtn.onclick = () => {
      if (selectedSlot !== null) {
        window.dropItem(selectedSlot);
      }
    };
  } else {
    useBtn.onclick = () => {
      if (selectedSlot !== null) {
        window.useItem(selectedSlot);
      }
    };
    dropBtn.onclick = () => {
      if (selectedSlot !== null) {
...
function updateTradeSlot(playerId, item) {
  ...
  // Обновляем состояние tradeState
  if (isInitiator) {
...
function placeItemInTradeSlot(slotIndex) {
  ...
  if (item.type === "balyary") {
    ...
    // Локально обновляем торговую ячейку
    updateTradeSlot(playerId, tradeItem);
    // Удаляем предмет из инвентаря локально
    inventory[slotIndex] = null;
    updateInventoryDisplay();
  }
}
