...
function initInventory(ws, myId, players) {
  ...
  inventoryContainer.insertBefore(inventoryGrid, document.getElementById("inventoryActions"));
  ...
  tradeBtn.addEventListener("click", (e) => {
    ...
    sendWhenReady(ws, JSON.stringify({ type: "tradeRequest", targetId: selectedPlayerId }));
...
function toggleInventory() {
  ...
  document.getElementById("inventoryBtn").classList.toggle("active", isInventoryOpen);
...
function selectSlot(slotIndex, slotElement) {
  ...
  if (tradeSession && !tradeSession.myItem) {
    if (!inventory[slotIndex].itemId) {
      inventory[slotIndex].itemId = `${inventory[slotIndex].type}_${Date.now()}_${slotIndex}`;
    }
    for (let i = 0; i < inventory.length; i++) {
      if (i !== slotIndex && inventory[i] && inventory[i].itemId === inventory[slotIndex].itemId) {
        inventory[i] = null;
      }
    }
    tradeSession.myItem = { ...inventory[slotIndex], slotIndex };
    inventory[slotIndex] = null;
    sendWhenReady(ws, JSON.stringify({
...
function useItem(slotIndex) {
  if (tradeSession) {
    ...
    if (effect.health) me.health = Math.min(100, Math.max(0, me.health + effect.health));
    if (effect.energy) me.energy = Math.min(100, Math.max(0, me.energy + effect.energy));
    if (effect.food) me.food = Math.min(100, Math.max(0, me.food + effect.food));
    if (effect.water) me.water = Math.min(100, Math.max(0, me.water + effect.water));
    inventory[slotIndex] = null;
    sendWhenReady(ws, JSON.stringify({
      type: "useItem",
      slotIndex,
  ...
  if (!tradeSession) {
    ...
    if (item.type === "balyary") {
      screen.innerHTML = `
        <div class="balyary-drop-form">
          ...
          <input type="number" id="balyaryAmount" class="cyber-input" min="1" max="${item.quantity || 1}" placeholder="0" value="" autofocus />
    ...
    if (tradeSession.myItem) {
      ...
      if (freeSlot !== -1) {
        ...
        document.getElementById("inventoryScreen").textContent = "Инвентарь полон, освободите слот!";
      }
    } else {
      sendWhenReady(ws, JSON.stringify({
        type: "tradeCancelled",
...
function showTradeRequest(fromId) {
  ...
  document.getElementById("acceptTradeBtn").addEventListener("click", () => {
    sendWhenReady(ws, JSON.stringify({ type: "tradeAccepted", targetId: fromId }));
    tradeSession = {
      partnerId: fromId,
      myItem: null,
      partnerItem: null,
  ...
  document.getElementById("declineTradeBtn").addEventListener("click", () => {
    sendWhenReady(ws, JSON.stringify({ type: "tradeDeclined", targetId: fromId }));
    tradeRequestDiv.remove();
    clearTimeout(tradeRequestTimeout);
  });
...
function updateTradeInventory() {
  ...
  if (tradeSession.myItem) {
    ...
    if (tradeSession.myItem.type === "balyary" && tradeSession.myItem.quantity > 1) {
      const quantityEl = document.createElement("div");
  ...
  if (tradeSession.partnerItem) {
    ...
    if (tradeSession.partnerItem.type === "balyary" && tradeSession.partnerItem.quantity > 1) {
      const quantityEl = document.createElement("div");
      quantityEl.className = "quantity";
    ...
    partnerTradeSlot.onmouseover = () => {
      screen.textContent = ITEM_CONFIG[tradeSession.partnerItem.type].description;
    };
  ...
  useBtn.textContent = "Обмен";
  useBtn.disabled = !tradeSession.myItem || tradeSession.myConfirmed || !tradeSession.partnerItem;
  dropBtn.textContent = "Отмена";
  dropBtn.disabled = false;
  updateInventoryDisplay();
}
...
function cancelTrade() {
  if (tradeSession) {
    if (tradeSession.myItem) {
      if (!inventory.some((slot) => slot && slot.itemId === tradeSession.myItem.itemId)) {
        const freeSlot = inventory.findIndex((slot) => slot === null);
        if (freeSlot !== -1) {
          inventory[freeSlot] = { ...tradeSession.myItem };
        } else {
  ...
  switch (data.type) {
    case "syncItems":
      ...
      data.items.forEach((item) =>
        items.set(item.itemId, { x: item.x, y: item.y, type: item.type, spawnTime: item.spawnTime })
      );
      data.items.forEach((item) => {
    ...
    case "itemPicked":
      ...
      if (data.playerId === myId && data.item) {
        if (data.item.type === "balyary") {
          const balyarySlot = inventory.findIndex((slot) => slot && slot.type === "balyary");
          if (balyarySlot !== -1) {
            inventory[balyarySlot].quantity = (inventory[balyarySlot].quantity || 1) + 1;
          } else {
            const freeSlot = inventory.findIndex((slot) => slot === null);
            if (freeSlot !== -1) {
              inventory[freeSlot] = {
                type: "balyary",
                quantity: 1,
                itemId: data.itemId || `${data.item.type}_${Date.now()}_${freeSlot}`,
              };
            }
          }
        } else {
          ...
          if (freeSlot !== -1) {
            inventory[freeSlot] = {
              type: data.item.type,
              itemId: data.item.itemId || `${data.item.type}_${Date.now()}_${freeSlot}`,
            };
          }
        }
        updateInventoryDisplay();
    ...
    case "update":
      ...
      break;
    case "itemDropped":
      items.set(data.itemId, { x: data.x, y: data.y, type: data.type, spawnTime: data.spawnTime });
      updateInventoryDisplay();
      break;
    case "tradeCompleted":
